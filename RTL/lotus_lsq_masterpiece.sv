`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// lotus_lsq_masterpiece - V3.8 STORE DRAIN ADDR/DATA VALIDITY FIX
// Engineer:     Sanuka Nethmira Amarasekara (Lotus Omni)
// Target:       Xilinx Artix-7 xc7a200t
//
// FIX V3.8 (THIS VERSION): Added addr_valid and data_valid checks to the
//   store drain logic. Previously, if the ROB committed a store before the
//   AGU finished computing its address (or if agu_rob_idx mismatched), the
//   LSQ would drain the store with addr=0, corrupting memory and causing
//   a pipeline deadlock.
//
// FIX V3.7: Removed restrictive 'in_flight' and 'valid' blocking conditions
//   on store drainage to guarantee prompt forwarding to memory subsystem.
//////////////////////////////////////////////////////////////////////////////////
...
always_comb begin
    ...
    // =================================================================
    // 🛠️ FIX V3.8: Direct Drain Store (Priority Push to L1D/DRAM)
    // MUST wait for addr_valid and data_valid to prevent draining
    // stores with addr=0 before AGU completes computation.
    // =================================================================
    if (n_sq_array[n_sq_head].valid && 
        n_sq_array[n_sq_head].committed && 
        n_sq_array[n_sq_head].addr_valid && 
        n_sq_array[n_sq_head].data_valid && 
        !head_fwd_match) begin
        n_l1d_req_valid             = 1'b1;
        n_l1d_req_rw                = 1'b1;
        n_l1d_req_addr              = head_addr_reg;
        n_l1d_req_data              = head_data_reg;
        n_l1d_req_wmask             = head_wmask_reg;
